...
#### additional certificate Subject Alternative Names (SANs)

The API server certificate can be configured to include additional certificate SANs if access
to the API server is required remotely, for example if you want to view your cluster on your local
network (Warning: see the NOTE above about exposing the kind cluster publicly).

Configuring the following will add the SANs to the list of SANs within the kubeadm generated
certificate (by setting the `apiServer.certSANs` value in the kubeadm configuration generated by kind):

{{< codeFromInline lang="yaml" >}}
kind: Cluster
apiVersion: kind.x-k8s.io/v1alpha4
networking:
  apiServerAdditionalCertSANs: [ "hostA", "hostB", "*.domain" ]
{{< /codeFromInline >}}
